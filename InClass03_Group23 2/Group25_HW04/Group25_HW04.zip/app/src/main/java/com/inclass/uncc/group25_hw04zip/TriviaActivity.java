...
public class TriviaActivity extends AppCompatActivity {
    ...
    Button btnnext,btnlast,btnQuit;
    Bitmap bm;
    int i = 1;
    int j = 0;
    boolean exit =false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView clockTextView =(TextView) findViewById(R.id.textView3);


        new CountDownTimer(120000, 1000) {

            public void onTick(long millisUntilFinished) {
                clockTextView.setText("Seconds Remaining: " + millisUntilFinished / 1000);
            }

            public void onFinish() {
                clockTextView.setText("done!");
                for (int m = 0; m < arr.size(); m++) {       //Log.d("resultar", "" + checkedanswers[m]);
                    if (checkedanswers[m] == arr.get(m).getAnswer()) {
