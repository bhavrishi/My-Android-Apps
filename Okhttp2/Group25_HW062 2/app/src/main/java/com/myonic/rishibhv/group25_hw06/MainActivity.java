...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    private void handlesigninrresult(final GoogleSignInResult result) {
        final boolean[] inloop = {false};
                    ...
                    for (DataSnapshot postsnapshot : dataSnapshot.getChildren()) {
                        if(postsnapshot.getKey().equals("name"))
                        {
                            ...
                            if(tempuser[0].equals(gaccnt.getDisplayName())) {
                                ...
                                i.putExtra(UID, gaccnt.getId());
                                i.putExtra(NAME, gaccnt.getDisplayName());
                                Log.d("demo", gaccnt.getId() + gaccnt.getDisplayName());
                                startActivity(i);
    ...
    databaseUsers.addValueEventListener(new ValueEventListener() {
       ...
       /* if (userAlreadyLoggedIn != null) {*/
           /* for (int i = 0; i < users.size(); i++) {
            ...
            Intent i = new Intent(MainActivity.this, HomeActivity.class);
            i.putExtra(UID, userAlreadyLoggedIn.getUid());
