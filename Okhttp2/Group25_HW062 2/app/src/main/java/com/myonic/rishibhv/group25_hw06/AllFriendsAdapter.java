...
public class AllFriendsAdapter extends RecyclerView.Adapter<AllFriendsAdapter.ViewHolder>{
    ...
    @Override
    public void onBindViewHolder(final AllFriendsAdapter.ViewHolder holder, final int position) {
        ...
        holder.tvfriendName.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                holder.hContext.callIntentToUserProfile( mdata.get(position).getId(),  mdata.get(position).getName());
            }
        });
    }
